...
for j = 1:length(spikes2) 
    ...
    potential(1) = LEAK_POTENTIAL;
    sra = zeros(size(tvec));
    Iapp2 = (randn(size(tvec)) * sigma/sqrt(dt)) + .1e-9;
    for i = 1:length(tvec)-1
       if potential(i) > THRESHOLD              
           potential(i) = RESET;                
           sra(i) = sra(i) + b;                 
...
stairs(centers1, probability1)
hold on
stairs(centers2, probability2)
title('Probability against Spike Count')
xlabel('Spike Count')
ylabel('Probability')
legend('- stimulus', '+ stimulus')

if (length(probability2) > length(probability1))
    temp = probability1;
    newlength = length(probability2) - length(probability1);
    zerosvec = zeros(1, newlength);
    probability1 = horzcat(temp, zerosvec);
end

figure;

plot(probability1, probability2)
title('ROC Curve')
xlabel('False +ve')
ylabel('True +ve')
